...
wire signed [63:0] mul_ss_product;
wire signed [63:0] mul_su_product;
wire        [63:0] mul_uu_product;
wire [31:0] mext_mul_result;
reg         mul_started;
reg         mul_done;
...
assign mul_ss_product =
    $signed({{32{alu_operand1[31]}}, alu_operand1}) *
    $signed({{32{alu_operand2[31]}}, alu_operand2});
assign mul_su_product =
    $signed({{32{alu_operand1[31]}}, alu_operand1}) *
    $signed({32'b0, alu_operand2});
assign mul_uu_product = {32'b0, alu_operand1} * {32'b0, alu_operand2};

assign mext_mul_result =
    (alu_op == 3'b000) ? mul_product[31:0]       : // MUL
    (alu_op == 3'b001) ? mul_ss_product[63:32]   : // MULH
    (alu_op == 3'b010) ? mul_su_product[63:32]   : // MULHSU
    (alu_op == 3'b011) ? mul_uu_product[63:32]   : // MULHU
                         32'hx;

// RV32M multiply group: MUL/MULH/MULHSU/MULHU have funct3 000-011.
wire is_mul_inst = alu && is_m_ext && (alu_op[2] == 1'b0);
wire new_mul_inst =
    is_mul_inst &&
...
always @(posedge clk or negedge reset) begin
    if (!reset) begin
        ...
        if ((is_mul_inst && mul_ready && !mul_ready_d) ||
            ...
            mext_result      <= is_div_inst ? div_result : mext_mul_result;
            mext_pc          <= pc;
	...
	case (1'b1)
        ...
        if (is_m_ext) begin
            if (is_div_inst) begin
                ...
                ex_result = mext_mul_result;   // MUL, MULH, MULHSU, MULHU
            end
        end 
